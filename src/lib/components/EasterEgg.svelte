<script lang="ts">
    ...
    function toggleOpenState(newOpenState: boolean, updateBGM?: boolean) {
        ...
        el.classList.toggle("open", newOpenState);
    }
</script>
...
<div class="easter-egg" bind:this={el}>
    <button class="handle-container" aria-label="Show easter egg" on:click={handleClick}>
        <img class="chain" role="none" src={chainImg} alt="" />
        <img class="handle" src={handleImg} alt="" role="none" />
    </button>

    <button tabindex={!el?.classList?.contains?.("open") ? -1 : 1} type="button" class="x" on:click={(ev) => ev.button === 0 && toggleOpenState(false)} title="Close">
